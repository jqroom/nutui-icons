...
const getSvgBackgroundTemplate = (
  viewBox: string,
  pathd: string[],
  name: string
) => {
    ...
    return h(
      props.tag,
      {
        ...
        style: {
          ...
          background: ${`${"`"}url("data:image/svg+xml,%3Csvg viewBox='${viewBox}' xmlns='http://www.w3.org/2000/svg'%3E${pathd.map(
            (path) => {
...
\n`;
  ...
  files.forEach((filepath: string) => {
    ...
    readFile(filepath, { encoding: "utf-8" }).then((res) => {
      ...
      outputFile(
        `${process.cwd()}/packages/icons-vue/src/components/${filename}.vue`,
        getSvgTemplate(viewBox, pathds, name),
        "utf8",
        (error) => {
      ...
      outputFile(
        `${process.cwd()}/packages/icons-vue-taro/src/components/${filename}.vue`,
        getSvgBackgroundTemplate(viewBox, pathds, name),
        "utf8",
        (error) => {
          consola.success(`${filename} 文件写入成功`);
        }
  ...
  outputFile(
    `${process.cwd()}/packages/icons-vue/dist/es/index.es.js`,
  ...
  outputFile(
    `${process.cwd()}/packages/icons-vue/src/buildEntry/lib-new.ts`,
    entryLib,
    "utf8",
    (error) => {
  ...
  outputFile(
    `${process.cwd()}/packages/icons-vue-taro/dist/es/index.es.js`,
    entryEs + 'import "../style_icon.css";',
    "utf8",
    (error) => {
...
`;
  ...
  entryTSC += s.join("\n");
  outputFile(
    `${process.cwd()}/packages/icons-vue-taro/dist/types/index.d.ts`,
    entryTSC,
    "utf8",
    (error) => {
  ...
  outputFile(
    `${process.cwd()}/packages/icons-vue/src/components/iconsConfig.ts`,
  ...
  outputFile(
    `${process.cwd()}/packages/icons-vue-taro/src/components/iconsConfig.ts`,
    `export const iconsConfig = ${JSON.stringify(entryArray)}`,
    "utf8",
    (error) => {
