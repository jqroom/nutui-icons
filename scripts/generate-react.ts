...
const Add:FunctionComponent<SVG_IconProps> = (props: SVG_IconProps) => {
  return <Icon {...props} name={props.name || '${compoentName}'} viewBox={'${viewBox}'}>
    ${d.map((d) => {
      return `<path d="${d}" fill="currentColor" fillOpacity="0.9"></path>`;
    })}
  </Icon>
}
...
new glob.Glob(pattern, {}, (err, files) => {
  ...
  fsExtra.outputFile(
    `${process.cwd()}/packages/${iconsReactTaroDir}/src/buildEntry/lib-new-dts.ts`,
    entryLibDTS,
    "utf8",
    (error) => {
